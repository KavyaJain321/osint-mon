...
// Translate all articles with title_en IS NULL using Groq.
// Runs in background — returns immediately. Check server logs for progress.
router.post('/migrate/translate-titles', async (req, res) => {
    try {
        ...
        res.json({
            message: `Translation started for ${count} articles. Running in background — check server logs for progress.`,
            total_to_translate: count,
        });

        translateMissingTitles().catch(e =>
            log.ai.error('[TitleTranslator] Fatal error in admin trigger', { error: e.message })
        );
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
